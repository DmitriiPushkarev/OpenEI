...
openai.api_key = "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"
...
# Перевод текста в аудио Text to Speech (TTS)
...
async def send(message: types.Message):
    response = openai.Completion.create(
        ...
        prompt= "Представь, что ты психолог и помоги мне со следующим вопросом:" + message.text,
        temperature=0.8,
        max_tokens=2000,
        top_p=1.0,
    ...
    out_filename = tts.text_to_ogg(response['choices'][0]['text'])
    # Отправка голосового сообщения
    path = Path("", out_filename)
    voice = InputFile(path)
    await bot.send_voice(message.from_user.id, voice)
...
async def voice_message_handler(message: types.Message):
    ...
    text = "Представь, что ты психолог и помоги мне со следующим вопросом:" + stt.audio_to_text(file_on_disk)
    if not text:
